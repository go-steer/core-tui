...
// metadata) at a leisurely cadence so the operator can watch the
// spinner / Glamour / per-turn footer in action.
func CodingDemo() []Step {
	chunk := 90 * time.Millisecond
	return []Step{
		{Wait: 200 * time.Millisecond, Event: tui.Event{Text: "Looking at the existing schema first ", Partial: true}},
		{Wait: chunk, Event: tui.Event{Text: "to confirm the current column definition.\n\n", Partial: true}},
		{Wait: 400 * time.Millisecond, Event: tui.Event{ToolCalls: []tui.ToolCall{
			{ID: "t1", Name: "Read", Args: map[string]any{"path": "db/schema/users.sql"}},
		}}},
		{Wait: 600 * time.Millisecond, Event: tui.Event{Text: "The `email` column is currently `VARCHAR(255)` ", Partial: true}},
		...
		{Wait: 300 * time.Millisecond, Event: tui.Event{ToolCalls: []tui.ToolCall{
			{ID: "t2", Name: "Write", Args: map[string]any{"path": "db/migrations/0042_users_email_not_null.sql"}},
		}}},
		{Wait: 500 * time.Millisecond, Event: tui.Event{ToolCalls: []tui.ToolCall{
			{ID: "t3", Name: "Bash", Args: map[string]any{"command": "psql -f db/migrations/0042_users_email_not_null.sql"}},
		}}},
		{Wait: 700 * time.Millisecond, Event: tui.Event{Text: "Done. The migration is at ", Partial: true}},
		...
		{Wait: 200 * time.Millisecond, Event: tui.Event{Usage: &tui.Usage{InputTokens: 8421, OutputTokens: 2103}}},
	}
}
